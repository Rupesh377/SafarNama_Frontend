...
import { Plus, Trash2, X, MapPin, Clock, Users } from "lucide-react";
import AdminLayout from "../../components/AdminLayout";
import { getAllCompletedEvents, createCompletedEvent, deleteCompletedEvent } from "../../Services/completedEventService";
import { uploadImage } from "../../Services/trekService";

const empty = { title: "", location: "", duration: "", price: "", imageUrl: "", description: "", itinerary: "", totalMembers: "" };
...
export default function ManageCompletedEvents() {
  ...
  const load = () => {
    getAllCompletedEvents().then((r) => setEvents(r.data)).catch(console.error).finally(() => setLoading(false));
  };
  useEffect(load, []);
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await createCompletedEvent({ ...form, price: Number(form.price), totalMembers: Number(form.totalMembers) });
      setForm(empty);
      setShowForm(false);
      load();
  ...
  return (
    <AdminLayout title="Completed Events" subtitle={`${events.length} event${events.length !== 1 ? "s" : ""}`} action={addBtn}>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 0", textAlign: "center" }}>
          ...
          <p style={{ color: "#333", fontWeight: 600, fontSize: "15px" }}>No completed events yet</p>
          <p style={{ color: "#999", fontSize: "13px", marginTop: "4px" }}>Click "Add Event" to build your archive.</p>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "20px" }}>
          {events.map((e) => (
            <div key={e.id} style={{ background: "white", borderRadius: "16px", border: "1.5px solid #e8ede6", overflow: "hidden", transition: "box-shadow 0.2s, transform 0.2s" }}
              ...
              <div style={{ padding: "16px" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "5px", marginBottom: "12px" }}>
                  ...
                  <span style={{ fontSize: "12px", color: "#777", display: "flex", alignItems: "center", gap: "6px" }}><Clock size={11} color="#4a7c59" />{e.duration}</span>
                </div>
                <div style={{ borderTop: "1px solid #f0f0ec", paddingTop: "10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  ...
                  <span style={{ fontSize: "11px", color: "#777", display: "flex", alignItems: "center", gap: "4px" }}><Users size={11} color="#4a7c59" />{e.totalMembers} members</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "16px" }}
          ...
          <div style={{ background: "white", borderRadius: "24px", width: "100%", maxWidth: "900px", maxHeight: "90vh", display: "flex", flexDirection: "column", boxShadow: "0 32px 80px rgba(0,0,0,0.15)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #eee", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
              <h2 style={{ fontSize: "18px", fontWeight: 800, color: "#1a1a1a" }}>Add Completed Event</h2>
            ...
            {/* Body — 2 cols */}
            <form onSubmit={handleSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", flex: 1, overflow: "hidden", minHeight: 0 }} className="modal-grid-c">
              <style>{`@media(max-width:640px){.modal-grid-c{grid-template-columns:1fr!important}} .adm-input-c{width:100%;border:1.5px solid #e8ede6;border-radius:10px;padding:10px 14px;font-size:14px;color:#1a1a1a;outline:none;transition:border-color 0.2s;background:#fafafa;} .adm-input-c:focus{border-color:#4a7c59;background:white;} .adm-input-c::placeholder{color:#bbb;} .adm-label-c{font-size:11px;font-weight:700;color:#4a7c59;letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px;display:block;}`}</style>

              {/* LEFT */}
              <div style={{ borderRight: "1px solid #f0f0ec", overflowY: "auto", padding: "24px", display: "flex", flexDirection: "column", gap: "14px" }}>
                <div>
                  <label className="adm-label-c">Event Title</label>
                  <input name="title" value={form.title} onChange={handleChange} required placeholder="e.g. Manali Trip 2024" className="adm-input-c" />
                </div>
                <div>
                  <label className="adm-label-c">Location</label>
                  <input name="location" value={form.location} onChange={handleChange} required placeholder="e.g. Himachal Pradesh" className="adm-input-c" />
                </div>
                <div>
                  <label className="adm-label-c">Duration</label>
                  <input name="duration" value={form.duration} onChange={handleChange} required placeholder="e.g. 3 Days / 2 Nights" className="adm-input-c" />
                </div>
                <div>
                  <label className="adm-label-c">Price (₹)</label>
                  <input name="price" type="number" value={form.price} onChange={handleChange} required placeholder="e.g. 6999" className="adm-input-c" />
                </div>
                <div>
                  <label className="adm-label-c">Total Members</label>
                  <input name="totalMembers" type="number" value={form.totalMembers} onChange={handleChange} required placeholder="e.g. 35" className="adm-input-c" />
                </div>
                <div>
                  <label className="adm-label-c">Image</label>
              ...
              <div style={{ overflowY: "auto", padding: "24px", display: "flex", flexDirection: "column", gap: "14px" }}>
                <div>
                  <label className="adm-label-c">Description</label>
                  <textarea name="description" value={form.description} onChange={handleChange} required rows={5}
                    placeholder="Describe the event experience..." className="adm-input-c" style={{ resize: "vertical" }} />
                </div>
                <div>
                  <label className="adm-label-c">Itinerary</label>
                  <textarea name="itinerary" value={form.itinerary} onChange={handleChange} required rows={5}
                    placeholder="Day 1: Arrival&#10;Day 2: Sightseeing&#10;Day 3: Departure" className="adm-input-c" style={{ resize: "vertical" }} />
                </div>
